        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    payload = f"[{timestamp}] INSEC->SEC {self.generate_payload(self.packet_size - 60)}"
    ...
    def send_tcp_traffic(self):
        ...
        while self.running:
            try:
                ...
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                payload = f"[{timestamp}] INSEC->SEC TCP {self.generate_payload(self.packet_size - 70)}"
...
def main():
    # Default settings
    target_ip = os.getenv('SECURENET_HOST_IP', '10.1.0.21')
    target_port = 8888
    protocol = 'tcp'
    interval = 1.0
    packet_size = 256
    
    print(f"INSEC Traffic Generator")
